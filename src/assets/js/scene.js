...
export class SceneViewer {
  ...
  updateLights(object) {
    ...
    const direction = this.directionalLight.reverse
...
export class LineSelector {
  ...
  onMouseDown(event) {
    if (event.button === 0) {
      ...
      if (this.start !== null) {
        ...
        this.line = new SelectionLine(
          this.scene,
          this.start.point,
          this.end.point
        );
        SelectionText.loadFont(
          "assets/fonts/helvetiker_regular.typeface.json",
          (font) => {
            this.text = new SelectionText(
              this.scene,
              this.start.point,
              font,
              "TEST"
  ...
  onMouseMove(event) {
    ...
    if (this.start !== null) {
      if (this.crossLine === null) {
        this.crossLine = new SelectionLine(
          this.scene,
          this.start.point,
          position
        );
...
export class ModelAnalyser {
    ...
    for (let n = 0; n < rays.length; n++) {
      ...
      for (let i = 0; i < row.length; i++) {
        ...
        if (
          ...
          ray.distance < this.min + this.depth
        ) {
          ray.showPoint();
        }
      }
  ...
  getValue(value) {
    if (
      value == Infinity ||
      value == undefined ||
      value > this.min + this.depth
    )
      return this.min + this.depth;
    return value;
  }
...
export class AnalysisRay extends THREE.Line {
  ...
  intersect(objects, draw = true) {
    ...
    const distance =
      intersections.length > 0 ? intersections[0].distance : Infinity;
