...
export class MyAccountComponent implements OnInit {
  ...
  comparePassword(event: any) {
    if (
      event.target.value != this.updatePasswordForm.controls['password'].value
    ) {
  ...
  updateEmail() {
    ...
    if (this.updateEmailForm.invalid) {
      ...
      let properties = {
        ...
        newEmail: this.updateEmailForm.controls['email'].value,
      };
      this.updateEmailModel = properties;
      this._registerService
        .updateEmail(this.updateEmailModel)
        .subscribe((result) => {
          if (result.statusCode == 200) {
            ...
            this.dialog.open(DialogConfirmComponent, {
              data: {
                title: 'Email Updated',
                message:
                  'An email was sent to activate your account.<br> Check your spam folder',
                buttonText: 'Ok',
              },
              width: '400px',
              panelClass: 'dialog',
            ...
            this.dialog.open(DialogConfirmComponent, {
              data: {
                ...
                message:
                  'Email already in use!<br> Please enter an another email to register the account',
                buttonText: 'Ok',
              },
              width: '400px',
              panelClass: 'dialog',
  ...
  updatePassword() {
    ...
    if (this.updatePasswordForm.invalid || this.isComparePassword) {
      ...
      this._registerService
        .updatePassword(this.loginModel)
        .subscribe((result) => {
          if (result.statusCode == 200) {
            this.dialog.open(DialogConfirmComponent, {
              data: {
  ...
  deleteAccount() {
    let modalDialogRef = this.dialog.open(DialogConfirmComponent, {
      data: {
        title: 'Delete Project',
        message:
          'Are you sure you want delete your account? <br> This action cannot be undone.',
        buttonText: 'No',
        buttonYesText: 'Yes',
        isShow: true,
      },
    ...
    modalDialogRef.afterClosed().subscribe((result) => {
      if (result) {
        ...
        if (token != null) {
          let tokenProp = this.authService.getDecodedAccessToken(token);
          this._registerService
            .deleteAccount(tokenProp.UserId)
            .subscribe((result) => {
              if (result.statusCode == 200) {
