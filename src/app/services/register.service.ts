...
import { LoginModel, ResetPasswordModel, UpdateEmailModel } from '../core/model/login.model';
...
export class RegisterService {
  ...
  public ActivateUserAccount(Code: string): Observable<any> {
    return this.http.post(`${BaseUrl.url}api/ActivateAccount?Code=${Code}`,null).pipe(
      map((response: any) =>
        response,
      ),
      catchError(err => {
  ...
  public deleteAccount(parameters: any): Observable<any> {
    return this.http.delete(`${BaseUrl.url}api/DeleteUserAccount/${parameters}`).pipe(
      map((response: any) =>
        response,
      ),
      catchError(err => {
